...
## 7. SAGA no Orquestrador

O fluxo `/prioritize` agora usa uma SAGA orquestrada:

* O Gateway gera ou repassa `X-Saga-Id`.
* O Orquestrador persiste o log em `orchestrator/saga_log.db`.
* As etapas registradas incluem `CLASSIFY_INTENT`, `FETCH_RAG`, `FETCH_TASKS`,
  `FETCH_CALENDAR`, `SAVE_PRIORITY_PLAN` e `GENERATE_RESPONSE`.
* O plano de priorizacao e salvo localmente em `priority_plans`.
* Em falha, a compensacao marca o plano como `CANCELLED` e remove recursos Google
  criados opcionalmente.
...
Para usar a criacao opcional de tarefas/eventos com compensacao, o OAuth precisa
autorizar tambem os escopos de escrita de Tasks e Calendar Events.
